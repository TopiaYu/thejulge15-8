...
const LatestNotice = ({ checkPoint }: { checkPoint?: string | null }) => {
  ...
  useEffect(() => {
    ...
    setLatestArray(storage ? JSON.parse(storage) : null);

    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'latest') {
        setLatestArray(e.newValue ? JSON.parse(e.newValue) : null);
      }
    };
  ...
  return (
    <>
      {!latestArray ? (
        ...
        <div className="flex flex-wrap justify-center sm:justify-start gap-2 sm:gap-3.5">
